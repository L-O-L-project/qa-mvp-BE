...
def test_run_geo_audit_propagates_crawl_error_detail(monkeypatch):
    async def fail_crawl(_url: str):
        raise RuntimeError('no pages crawled: https://optiflow.kr/geo: ProxyError: 403 Forbidden')

    monkeypatch.setattr('app.analysis.pipeline.crawl_site', fail_crawl)

    try:
        asyncio.run(run_geo_audit('https://optiflow.kr/geo'))
    except RuntimeError as exc:
        assert 'ProxyError' in str(exc)
    else:
